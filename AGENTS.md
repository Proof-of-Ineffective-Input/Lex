...
- MCP tool names: [`web_search_lex`](main.go:48) / [`web_fetch_lex`](main.go:55) — Exa-style naming, aliased as `search`/`web_search` and `fetch`/`web_fetch` respectively
...
- Tool args:
  - [`SearchArgs`](main.go:21) — `query` + optional `description` (dual-query reranking) + `max_results` clamped to `[5, 50]`, default 10
  - [`FetchArgs`](main.go:27) — `urls` list + `limit` clamped to `[2000, 64000]`, rounded to nearest 1000; `0` means no truncation
